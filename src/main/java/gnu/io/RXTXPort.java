...
final public class RXTXPort extends SerialPort {
	/*
	 * I had a report that some JRE's complain when MonitorThread tries to
	 ...
	 * 
	 *      If speed is not a predifined speed it is assumed to be the actual
	 *      speed desired.
	 */
	...
	public void removeEventListener() {
		...
		if (monThreadisInterrupted == true) {
			...
			try {
				...
				// we dont propagate InterruptedExceptions so lets re-set the
				// flag
	...
	*/
	...
	class SerialInputStream extends InputStream {
		 ...
		 *      threshold = 0, blocks until timeout occurs, returns -1 on
		 *      timeout >0 >0 blocks until timeout, returns - 1 on timeout,
		 *      magnitude of threshold doesn't play a role. 0 >0 Blocks until 1
		 *      byte, magnitude of threshold doesn't play a role
		 */
		 ...
		 *             timeout threshold Behavior
		 *             ---------------------------------------------------------
		 *             --------------- 0 0 blocks until 1 byte is available >0 0
		 *             blocks until timeout occurs, returns 0 on timeout >0 >0
		 *             blocks until timeout or reads threshold bytes, returns 0
		 ...
		 * http://java.sun.com/products/jdk/1.2/docs/api/java/io/InputStream.
		 * html#read(byte[], int, int)
		 */
		/**
		 * @param b[]
		 * @param off
		 ...
		 *             blocks until timeout occurs, returns 0 on timeout >0 >0
		 *             blocks until timeout or reads threshold bytes, returns 0
		 *             on timeout 0 >0 blocks until either threshold # of bytes
		 *             or len bytes, whichever was lower.
		 */
		public synchronized int read(byte b[], int off, int len) throws IOException {
			if (debug_read)
				z.reportln("RXTXPort:SerialInputStream:read(" + b.length + " " + off + " " + len
						+ ") called" /* + new String(b) */ );
			int result;
			/*
			 * Some sanity checks
		 ...
		 * 
		 *             if the read encounters the two bytes, it will return and
		 *             the array will contain the terminator. Otherwise read
		 *             behavior should be the same as read( b[], off, len ).
		 *             Timeouts have not been well tested.
		 */
